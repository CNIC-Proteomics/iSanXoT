...
def extract_proteins(df, tagDecoy):
    ...
    df["Protein"],df["Protein_Description"] = zip(*[ s[0] for s in da ])
    # create the list of proteins of redundancy (the rest of proteins)
    df["Protein_Redundancy"] = [ ";".join([i[0] for i in s[1:] if i[0]]) for s in da ]
    return df
...
def main(args):
    ...
    logging.info("extract the proteins list discarding DECOY proteins (by experiment)")
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.n_workers) as executor:        
        ddf = executor.map(extract_proteins, list(ddf.groupby("Experiment")), repeat(args.lab_decoy))
    ddf = pd.concat(ddf)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Calculate the FDR and get the first protein selected alphabetically',
        epilog='''
        Example:
