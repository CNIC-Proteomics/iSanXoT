...
Usage: aljamia.py -x[xml file] -i[fold field] [-j[weight field] -k[id string], ...] [OPTIONS]
   ...
   -e, --remove-empty
                       Remove rows depending on which column is empty.
					   Example:
					   		- "0" value, remove the rows when 
							   the first column is empty.
							- "0,1" value, remove the rows when
								the first OR second columns are empty.
					   By default is False.
...
# end: jmrc
	...
	# begin: jmrc
	if removeEmptyRows:
		result = stats.removeEmptyRows(result, removeEmptyRows)
	# end: jmrc
